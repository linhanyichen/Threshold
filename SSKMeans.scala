...
/**
  * DRG亚组聚类计算器
  * 聚类中心点按费用值由低到高排序
  * (1) 四个聚类中心点时分别对应5,3,2,1
  * (2) 三个聚类中心点时分别对应5,3,2
  * (3) 两个聚类中心点时分别对应5,2
  * (4) 不设亚组则以主干组命名XXX9
  * create by zhuy 2019-11-07
  */
class SSKMeans(spark:SparkSession) {
  ...
  private [this] var dataSavePath:String = ""
  ...
  /**
    ...
    */
  def runSSKmeans(mCode:String,cNumber:Int) :Unit = {
    mainCode = mCode
  ...
  private def getSamples() :Unit = {
    ...
    subGroupFeeDF = spark.read.parquet(subFeeDataFilePath).select("`子集码`","`中位数`")
    var fieldMap:Map[String,String]  = Map.empty
    subGroupFeeDF.schema.foreach(field => {
      val dataType = field.dataType.toString.replaceAll("Type","")
      fieldMap ++= Map(field.name -> dataType)
    })

    var iLoop:Int = 0
    val cols:util.LinkedList[util.HashMap[String, Object]] = DFUtil.DF2List(subGroupFeeDF,fieldMap)

    LogUtil.logDEBUG(s"待处理的数据样本个数为[${cols.size()}]个")

    samples = cols.toArray.map(col => {
      val code:String = col.asInstanceOf[util.HashMap[String, Object]].get("子集码").toString
      val midFee:Double = col.asInstanceOf[util.HashMap[String, Object]].get("中位数").toString.toDouble
